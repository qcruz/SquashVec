...
| **Universal Basic Income (Negative Income Tax)** | `proposal/universal-basic-income.md` | UBI implemented as a Negative Income Tax — universal refundable credit delivered via the tax return system, with a 30% phase-out rate and monthly TRAL advance loans against the guaranteed annual entitlement. Eliminates poverty traps and benefit cliffs, replaces fragmented means-tested programs, and automatically scales with AI displacement via ATC revenue linkage. Net new cost ~$89B/year after replaced program savings and ATC offset (~0.3% of GDP). Phase 3 normalization instrument for AI unemployment scenario. |
